...
namespace WebMarketTests.TestHelpers
{
    internal static class DbHelper
    {
        public static DbContextOptions<WebMarketDbContext> GetTestDbOptions()
        {
            var options = new DbContextOptionsBuilder<WebMarketDbContext>()
                .UseInMemoryDatabase("testingDB")
                .Options;

            using (var context = new WebMarketDbContext(options))
            {
                SeedData(context);
            }
        ...
        private static void SeedData(WebMarketDbContext context)
        {
